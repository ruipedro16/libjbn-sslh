...
void params_set_and_check_R(mpz_t R, const mpz_t P, size_t NLIMBS) {
    ...
    // mpz_cmp: "returns a positive value if op1 > op2 ; zero if op1 = op2 ; negative value if op1 <
    // op2"
    mpz_gcd(gcd, R, P);
...
void params_precompute_all(uint64_t **P, uint64_t **RmP, uint64_t **Pm2, uint64_t **RmodP,
                           uint64_t **R2modP, uint64_t *u0,

                           int dest_init, char *Pstr, size_t NLIMBS) {
    if (dest_init == 1) {
...
void params_print_c(uint64_t *P, uint64_t *RmP, uint64_t *Pm2, uint64_t *RmodP, uint64_t *R2modP,
                    uint64_t u0, size_t NLIMBS, FILE *fout) {
...
int main(int argc, char **argv) {
    ...
    if (argc != 4) {
        ...
        fprintf(stderr,
                "example: ./params j 7 "
                "0x0002341f271773446cfc5fd681c520567bc65c783158aea3fdc1767ae2ffffffffffffffffffffff"
